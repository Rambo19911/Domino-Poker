...
- **Multiplayer (live):** authoritative WebSocket server with a lobby, rooms, chat, server-driven turn timers, reconnect/resume, SQLite/PostgreSQL persistence (match event log + player stats + chat history), and PostgreSQL-backed multi-instance foundations (room leases, durable sessions, cross-instance fanout). The main lobby exposes a live multiplayer entry alongside single-player.
