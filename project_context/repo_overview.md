...
- `apps/web/app/globals.css`: @import-only CSS entry; all styles live in `apps/web/styles/` per-feature partials (tokens, base, lobby, sp-table, tiles, info-panel, effects, settings, dialogs, animations, responsive, mp-lobby, mp-game, mp-mobile-table, mp-mobile-lobby, lobby-profile, auth). Import order = cascade order; `styles/tokens.css` is the single design-token source (colors, radius scale, z-index layers).
