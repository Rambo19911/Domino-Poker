...
Last refreshed: 2026-06-21 (deep player statistics — bid accuracy + placement distributions for registered users; new `player_game_results` table / migration 0008, `PlayerStatsStore`/`PlayerStatsService`/`MpStatsRecorder`, `ROUND_RESULT.playerResults` enrichment, `POST /sp/complete`, `GET /stats`, profile "Statistics" tab). Earlier: v3.2 swappable per-screen SVG backgrounds + per-screen palette accents + `--felt` token; gold-coin Phase 5.
...
- `apps/server`: authoritative multiplayer server, HTTP `/health`, `/metrics`, optional `/auth/*`, optional `/sp/*` (single-player coin rewards + `/sp/complete` stats), optional `/stats` (deep player statistics), optional `/contact` (About-panel contact form → owner email via Resend; no auth, IP rate-limited), optional `/chat/translate`, WebSocket `/ws`, gateway/hub/fanout, room/lobby lifecycle, game timers/directors, chat, auth, the gold-coin wallet (`wallet/WalletService.ts`, `storage/CoinStore.ts`) + SP reward tokens (`sp/SpRewardTokens.ts`), the player-stats services (`stats/PlayerStatsService.ts`, `stats/MpStatsRecorder.ts`, `storage/PlayerStatsStore.ts`), sessions/identity, storage adapters, PostgreSQL event bus, and server tests.
