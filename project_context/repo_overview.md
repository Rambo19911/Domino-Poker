...
Last refreshed: 2026-06-22 (Admin panel Phase 0 — separate Game Manager portal: new `apps/admin` Next.js app on port 3001 + new `/admin/*` routes in the existing server, English-only. Server `apps/server/src/admin/` module: `AdminStore` capability (impl in both DB adapters), `AdminAuthService` (scrypt password + Resend email OTP 2FA, cookie+CSRF sessions), `AdminAuditService`, `adminRoutes`, `cookies`; migration `0009_admin` (admin_sessions, admin_login_codes singleton, admin_audit_log, login_attempts); admin config in `config.ts`; player login_attempts via `authRoutes.onLoginAttempt`. Codex-reviewed PASS. Phases 1-4 not yet built; deploy/Caddy subdomain deferred. See `docs/TODO/admin-panel-plan.md`.) Earlier: 2026-06-21 deep player statistics (`player_game_results` / migration 0008); v3.2 swappable per-screen SVG backgrounds; gold-coin Phase 5.
...
- `apps/server/src/storage/schema.ts`: single DDL source for SQLite and PostgreSQL migrations (latest is `0008_player_game_results`: per-game stats rows with a `mode` sp/mp discriminator + CHECK constraints; previous `0007_coin_wallet`: `coin_balances` + append-only `coin_ledger`). Add migrations only at the end; do not renumber or fork schema definitions.
