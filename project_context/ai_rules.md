...
- The bot drives single-player CPU seats. The B-part MP hint ALSO runs this same ISMCTS engine CLIENT-side (via `apps/web/lib/mp/botView.ts` → `botBridge.decideMoveFromView`, reusing the `apps/web/lib/bot/playerView.ts` conversions, D10) to compute the recommended move from the player's own `PlayerSnapshot` (D8). But multiplayer *opponent* bots are still core `aiService` via `core/multiplayer` `autoBid`/`autoMove` -> server `GameDirector` — that heuristic is NOT dead code; do not delete it.
- `apps/web/lib/bot/playerView.ts` is the SINGLE source for core-state → ai_bot `PlayerView` conversions (D10). Keep `TrickSourceState` minimal so BOTH core `GameState` (SP) and MP `PlayerSnapshot` satisfy it structurally. Do NOT re-inline these conversions in SP or MP; extend `playerView.ts`. SP `buildPlayerView` output must stay byte-identical after the extraction (tested in `botView.test.ts`).
...
- Store (coin sink): item ownership is DERIVED from the append-only `coin_ledger` (`UNIQUE(user_id, reason, ref)`), NOT a separate inventory table — one purchase is a single atomic ledger row that is simultaneously the coin debit AND the ownership grant, so it is exactly-once and a re-buy is an idempotent no-op (`applied=false` = already owned). The catalog now has TWO product kinds: `theme` (6 themes, reason `theme_purchase`) and `bot_assistant` (the supportHuman bot "The Sage", reason `bot_purchase`); `StoreService`'s `KIND_TO_REASON` maps kind→reason, so add a new product by extending `StoreItemKind` + the map, NOT by hardcoding a reason. `WalletService.listOwnedItems` unions ALL ownership reasons — so consumers must FILTER `/store/owned` by kind/id for their concern (theme code matches `theme.*` ids via `isThemeUnlocked`; bot code uses `ownsSupportHuman`) — a foreign ref is inert, never assume the list is one kind. `packages/shared/src/store-catalog.ts` is the authoritative item+price source (prices from `economy.ts`: `THEME_PRICE`, `BOT_ASSISTANT_PRICE`); the server validates the itemId/price and rejects unknown items — never trust a client-supplied price. Keep purchase logic in `WalletService.purchaseItem` / `StoreService`; the web only displays catalog prices/locks and reconciles state against `/store/owned`. SP bot hint (owned `bot.supportHuman` → 3/round): reuse `botBridge.decideMove(state, humanSeat, "epic")` (no new bot engine code), keep the quota in React state (core stays pure), validate the recommendation with `tryAdvance` before consuming, and preserve the FULL move `{tile, declaredNumber}` (declared pip matters for lead moves). MP hints (Part B, BUILT + Codex-PASS, uncommitted) keep the quota SERVER-authoritative in `RoomEngine.requestHint`/`syncHintRound` (ephemeral, out of core state; grants-only idempotence cache keyed `playerId:turnId:requestId` — the turnId is a security guard: without it a client could replay one grant across turns to bypass the 3/round cap) and the ownership check in the ROUTER (`messageRouter.handleRequestHint` via `wallet.listOwnedItems`+`ownsSupportHuman`), NOT in `RoomEngine`. The hint MOVE is computed client-side (`useMpHint` orchestrates request → `HINT_GRANTED` → `computeMpHint` epic worker → highlight, with D9 requestId-correlation, D11 turnId-recheck, and a synchronous double-spend guard; the button renders only when `useOwnsSupportHuman` is true). The client-side compute bypass is an ACCEPTED, documented risk (owner + Codex, 2026-07-04): a client "block engine load" gate is security theater (the ISMCTS ships for SP anyway); the server ownership+quota gate is sufficient for a free-to-play coin game with no cashout. If ever needed, server-side hint compute from the player-perspective view is the robust path.
...
Coin-earning daily tasks (calendar icon in the lobby top bar, registered-users-only). Read before touching: shared `packages/shared/src/economy.ts` (`DAILY_TASKS` catalog + `DailyTaskId`), server `apps/server/src/daily/DailyTaskService.ts`, `http/dailyTaskRoutes.ts`, `wallet/WalletService.ts` (`listDailyTaskClaims`/`creditDailyTaskReward`), `storage/PlayerStatsStore.ts` (`countSpWinsSince` + `durationMs`), `storage/schema.ts` (migration `0014`); web `apps/web/lib/daily/*`, `components/DailyTasksDialog.tsx`, `components/LobbyScreen.tsx`, `styles/daily.css`. Full plan: `docs/TODO/daily-tasks-plan.md`.

- 4 SEQUENTIAL tasks: win ONE single-player game (placement ≤ 2) at a difficulty with `round_count >= requiredRounds` — 10-round medium→2000, 20-round hard→4000, 30-round epic→8000, 50-round epic→16000. `>=` semantics: a single 50-round epic win satisfies BOTH the 30- and 50-round epic tasks. DAILY reset UTC 00:00; progress is BINARY (0/1, parallel), CLAIM is sequential (1→2→3→4 within the same UTC day). (Reworked 2026-07-04 from the old "win N separate games" model — the round count is what escalates, not the number of wins.)
- Progress is DERIVED from `player_game_results` (`countSpWinsSince(userId, difficulty, windowStart, windowEnd, minDurationMs, minRounds)` — counts wins with `round_count >= minRounds`; round count is SERVER-AUTHORITATIVE, snapshotted in the `/sp/start` token), NO counter table. `windowStart = max(utcDayStart, FEATURE_LAUNCH_EPOCH_MS)`. Claims are ledger rows (`reason "daily_task_reward"`, ref `daily:{yyyymmdd}:{taskId}`, idempotent by `UNIQUE(user_id, reason, ref)` — no migration for the reason, 0010 dropped the CHECK). Amounts/`requiredRounds` ONLY in `economy.ts` (server-enforced; client never sends them). The `DailyTaskService` win-counter cache keys by `difficulty:minRounds` (epic tasks 3 (≥30) and 4 (≥50) need different gates, so difficulty alone is not a safe cache key).
- Migration `0014_player_game_results_duration` is ADDITIVE (`duration_ms` nullable column + index `(user_id, mode, difficulty, completed_at)`). `SchemaMigration.selfTransaction` marks the SQLite rebuilds (0010/0013) that manage their own tx; the SQLite `migrate()` runner now wraps every OTHER migration's `up` + its `schema_migrations` record in ONE `BEGIN IMMEDIATE` tx (0014 `ADD COLUMN` has no `IF NOT EXISTS`, so this makes it crash-rerun safe). `/sp/complete` stores `durationMs = now - token.issuedAt` (clamp ≥ 0) on the same idempotent insert (dup does NOT update it).
- ANTI-ABUSE: min-game-duration-per-difficulty gate (`DAILY_TASK_MIN_GAME_MS`, server-only) on what counts + the natural 30 000/day/user cap (each task claimable once per UTC day) + per-user rate limits + a per-user in-process claim lock (like `WalletService`; ledger UNIQUE is the real guard). RESIDUAL (documented, accepted like existing SP rewards): SP `placement` is still client-asserted at `/sp/complete` — the duration gate blocks instant scripts, not fabricated placements; full fix = server-side game replay (out of scope).
- Routes `GET /daily/tasks` + `POST /daily/tasks/claim` (auth required, anon 401, rate-limited, bearer-token so no CSRF) chain in `httpServer.ts` BEFORE the auth handler; mounted only when wallet + `PlayerStatsStore` + `AuthService` exist. Like every new server route, needs a Caddy `reverse_proxy /daily/*` rule on the VPS or it 404s in prod.
- Web: `useDailyTasks` fetches on lobby mount + when `dailyRefreshSignal` (bumped by AppShell after `/sp/complete`) changes; has a `reqRef` sequence guard so a stale fetch cannot overwrite newer state. The calendar icon shows only when authed and pulses (`--coin` glow, reduced-motion-safe) when `anyClaimable`. Countdown is computed locally from `secondsUntilReset` (never re-fetched). Token-only CSS.

### Weekly Tasks

Second tab in the SAME lobby dialog as Daily Tasks (the calendar icon opens a tabbed `Daily | Weekly` dialog). Registered-users-only. Read before touching: shared `packages/shared/src/economy.ts` (`WEEKLY_TASKS` catalog, `SpVariant`, `WeeklyTaskKind`, `WEEKLY_TASK_PLACEMENT_MAX`, `WeeklyTaskId`), server `apps/server/src/weekly/WeeklyTaskService.ts`, `http/weeklyTaskRoutes.ts`, `wallet/WalletService.ts` (`listWeeklyTaskClaims`/`creditWeeklyTaskReward`), `storage/PlayerStatsStore.ts` (`countMpFinishedSince` + `countSpTaskWins`), `storage/schema.ts` (migration `0015` — `variant` column), `sp/SpRewardTokens.ts` + `http/spRewardRoutes.ts` (`variant` plumbing); web `apps/web/lib/weekly/*`, `components/DailyTasksDialog.tsx` (tabbed), `components/AppShell.tsx` (`spSpecial` + `startWeeklyBossRoom`), `styles/daily.css`. Full plan: `docs/TODO/weekly-tasks-plan.md`.

- 4 INDEPENDENT tasks (claim each in any order — NO sequential lock, unlike daily), WEEKLY reset Monday 00:00 UTC, COUNT-based progress `min(threshold, count)`: (1) `mp_finish_20` — finish 20 MP games (any placement) → 40 000; (2) `sp_epic50_x2` — win TWO standard-room epic 50-round games → 100 000; (3) `boss30` — take a winning place (top-2) in a 30-round SPECIAL-room game → 150 000; (4) `boss50` — same at 50 rounds → 400 000. "Win"/"winning place" = `placement <= WEEKLY_TASK_PLACEMENT_MAX` (2, top-2). Round match is EXACT (`round_count == exactRounds`), not `>=`.
- SPECIAL ROOM = the mixed-bot "weekly bosses" SP room (tasks 3/4 only). Marked by `player_game_results.variant = 'weekly_bosses'` (migration `0015`, nullable TEXT; NULL = standard/legacy/MP). Plumbed server-authoritatively: `/sp/start` optional `variant` → `SpRewardTokens` snapshot → `/sp/complete` → `recordSpGame`. Task 2 counts ONLY standard rooms (`variant IS NULL`); tasks 3/4 count ONLY `weekly_bosses` — deliberate separation (owner decision). The special room's 3 bots have DIFFERENT behaviors (`AppShell.WEEKLY_BOSS_BEHAVIORS = [inclusion(human seat, ignored), inclusion, denyHuman, aggressiveVsHuman]`, all epic budget) — this is what wired `docs/bot-behaviors.md` (see AI Bot Package section). `startWeeklyBossRoom` uses `spSpecial` state so it never overwrites the user's persisted difficulty/round choice.
- Progress DERIVED from `player_game_results`, NO counter table: `countMpFinishedSince(userId, sinceMs, untilMs)` (mode='mp' rows in window) and `countSpTaskWins(userId, difficulty, variant|null, exactRounds, sinceMs, untilMs, minDurationMs, placementMax)` (`variant IS NULL` vs `variant = ?` is a two-branch SQL split — NULL is not `=`-comparable). Two indexes (0015): `idx_pgr_sp_weekly (user_id, mode, difficulty, variant, round_count, completed_at)` and `idx_pgr_mp_completed (user_id, mode, completed_at)`. PG `ADD COLUMN IF NOT EXISTS` (runner runs `up` + `schema_migrations` in SEPARATE tx). Claims are ledger rows (`reason "weekly_task_reward"`, ref `weekly:{mondayYYYYMMDD}:{taskId}`, idempotent by `UNIQUE(user_id, reason, ref)`). Amounts/thresholds/rounds ONLY in `economy.ts`.
- UTC-Monday window: `utcWeekStart(now) = utcDayStart - ((getUTCDay()+6)%7)*DAY_MS`, `weekEnd = weekStart + 7d`, `windowStart = max(weekStart, FEATURE_LAUNCH_EPOCH_MS)`, weekKey = Monday `yyyymmdd`. **INVARIANT: `FEATURE_LAUNCH_EPOCH_MS` (weekly) MUST be a Monday ≤ the current week, else the current-week window is EMPTY (`windowStart == weekEnd`) and NOTHING counts.** This was a real bug (2026-07-05): the constant was `2026-07-06` (next Monday) → empty window → no game counted; fixed to `2026-06-29`. On DEPLOY set it to the deploy week's Monday.
- ANTI-ABUSE (risk model identical to daily/SP — `placement` + `variant` are client-declared): SP tasks use a ROUND-SCALED min-duration gate `max(WEEKLY_TASK_MIN_GAME_MS_FLOOR=8000, WEEKLY_TASK_MIN_MS_PER_ROUND=1000 * exactRounds)` → 30s for boss30, 50s for boss50/epic50 (owner's real epic-50 pace ~72s, so legit play passes; blocks instant scripts). MP task has NO min gate (owner decision 2026-07-05: small 40k reward, and free MP rooms exist so `mp_entry` is not a reliable limiter — 1-round games count). Natural weekly cap = 40k+100k+150k+400k = 690 000/week/user (4 once-per-week claims). Per-user rate limits + in-process claim lock (ledger UNIQUE is the real guard). RESIDUAL (documented, accepted like SP rewards): SP `placement` AND `variant` are client-asserted at `/sp/start`/`/sp/complete` — the duration gate blocks instant scripts, not fabricated placement/variant; full fix = server-side game replay (out of scope).
- Routes `GET /weekly/tasks` + `POST /weekly/tasks/claim` (auth required, anon 401, rate-limited, bearer-token) chain in `httpServer.ts` AFTER daily, BEFORE auth; mounted only when wallet + `PlayerStatsStore` + `AuthService` exist. Needs a Caddy `reverse_proxy /weekly/*` rule on the VPS or it 404s in prod (owner authorized editing the Caddyfile).
- Web: `useWeeklyTasks` (mirrors `useDailyTasks`, `reqRef` guard) + `useDailyTasks` BOTH read the shared `tasksRefreshSignal` (renamed from `dailyRefreshSignal`; bumped after `/sp/complete`). MP-finish progress refreshes via LobbyScreen remount-fetch on returning from the `mp-lobby` screen (MP requires leaving the lobby; no separate MP GAME_OVER bump). The tabbed dialog reuses `.settingsTab` tokens (`role="group"`+`aria-pressed`) and a shared `TaskCard` (parametrized `progress/threshold` bar). Boss tasks show a `[Play]` button → `onPlayWeekly(exactRounds)` → `startWeeklyBossRoom`. The daily card title was shrunk (0.85rem, non-bold) to fit the tab row. 11 i18n keys NATIVELY translated across all 21 locales; the orphaned `dailyTasks` key was removed (lobby button uses `tasks`). Token-only CSS + `flex-wrap` tabs for @375px.
- TESTS: `apps/server/test/weekly/WeeklyTaskService.test.ts` (unit), `weeklyTaskRoutes.integration.test.ts` (routes), `packages/shared/test/weekly-tasks.test.ts` (catalog invariants), and `apps/server/test/weekly/weeklyTaskFlow.integration.test.ts` (end-to-end SIMULATION: real /auth+/sp+/weekly handlers + controllable clock using the REAL default epoch — covers all 4 tasks incl. boss 2nd-place + 1-round MP + a fixed-date epoch-regression guard). Live smoke script: `scripts/sim-weekly-tasks.mjs` (real delays vs a running server, SP tasks). Storage counters covered in the storage contract (SQLite + PG).
