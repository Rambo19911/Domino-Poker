...
- 4 SEQUENTIAL tasks (win 10 medium→2000, 20 hard→4000, 30 epic→8000, 50 epic→16000; win = SP placement ≤ 2). DAILY reset UTC 00:00; progress accrues in parallel, CLAIM is sequential (1→2→3→4 within the same UTC day). Tasks 3+4 share the same epic win count.
- Progress is DERIVED from `player_game_results` (`countSpWinsSince(userId, difficulty, windowStart, windowEnd, minDurationMs)`), NO counter table. `windowStart = max(utcDayStart, FEATURE_LAUNCH_EPOCH_MS)`. Claims are ledger rows (`reason "daily_task_reward"`, ref `daily:{yyyymmdd}:{taskId}`, idempotent by `UNIQUE(user_id, reason, ref)` — no migration for the reason, 0010 dropped the CHECK). Amounts/thresholds ONLY in `economy.ts` (server-enforced; client never sends them).
