...
- Web: `useDailyTasks` fetches on lobby mount + when `dailyRefreshSignal` (bumped by AppShell after `/sp/complete`) changes; has a `reqRef` sequence guard so a stale fetch cannot overwrite newer state. The calendar icon shows only when authed and pulses (`--coin` glow, reduced-motion-safe) when `anyClaimable`. Countdown is computed locally from `secondsUntilReset` (never re-fetched). 9 i18n keys across all 21 locales (EN+LV proper, 19 EN fallback). Token-only CSS.
